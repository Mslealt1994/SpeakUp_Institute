    ...
    <section className="py-8 sm:py-24">
      <Container className="max-w-6xl">
        <div className="flex flex-col items-center gap-12 lg:flex-row lg:items-center lg:gap-16">
          ...
          {/* ── Texto — izquierda ── */}
          <div className="flex-1 bg-white/50 p-6 rounded-2xl
            ...
            */}
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center lg:justify-start">
              <Link
                href="/blog"
                className="w-full sm:w-auto px-7 py-3 text-center rounded-full
                           bg-primary text-white font-bold
                           shadow-lg transition-colors hover:bg-primary-dark"
              >
                Cada paso cuenta
              </Link>
  ...
  return (
    ...
    >
      ...
      {[110, 126, 142, 158, 174, 190, 206].map((y, i) => (
        <rect key={`left-${i}`} x="100" y={y}
          width={[100, 80, 100, 60, 90, 75, 100][i]}
          height="6" rx="3" fill="white" opacity="0.3" />
      ))}

      {/* Líneas página derecha */}
      {[110, 126, 142, 158, 174, 190, 206].map((y, i) => (
        <rect key={`right-${i}`} x="278" y={y}
          width={[100, 70, 100, 85, 55, 100, 65][i]}
          height="6" rx="3" fill="white" opacity="0.3" />
      ))}

      {/* Palabras destacadas */}
      <rect x="100" y="230" width="50" height="14" rx="3" fill="#58CC02" opacity="0.7" />
      <rect x="158" y="230" width="42" height="14" rx="3" fill="#58CC02" opacity="0.4" />
      ...
      <text x="96" y="53" textAnchor="middle"
        fill="white" fontSize="14" fontWeight="bold" fontFamily="serif">
        A→C
      </text>

      {/* Pluma */}
      <rect x="370" y="190" width="12" height="60" rx="4"
        fill="#0B3C5D" transform="rotate(-20 370 190)" />
      <polygon points="370,248 382,248 376,268"
        fill="#082D46" transform="rotate(-20 376 258)" />
      <rect x="370" y="190" width="12" height="10" rx="2"
        fill="#58CC02" opacity="0.8" transform="rotate(-20 370 190)" />
      ...
      <text x="280" y="337" textAnchor="middle"
